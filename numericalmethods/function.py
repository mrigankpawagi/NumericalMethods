...
class FEM2D:
    """
    Solves the 2D Poisson equation on a rectangular domain using the Finite Element
    Method with linear triangular elements.

    -Δu(x,y) = f(x,y)  on  Ω = [xa, xb] × [ya, yb]
     u(x,y)  = g(x,y)  on  ∂Ω  (Dirichlet boundary condition)
    """

    def __init__(self, f: BivariateFunction, g: BivariateFunction,
                 xa: float, xb: float, ya: float, yb: float):
        """
        f: source term f(x, y)
        g: Dirichlet boundary condition g(x, y)
        xa, xb: x-domain bounds
        ya, yb: y-domain bounds
        """
        self.f = f
        self.g = g
        self.xa = xa
    ...
    def solve(self, nx: int = 4, ny: int = 4) -> BivariateFunction:
        """
        Solve using linear triangular finite elements on a uniform mesh.

        nx: number of subdivisions in the x-direction
        ny: number of subdivisions in the y-direction

        Returns a BivariateFunction approximating the solution u(x, y).
        """
        return self.solve_linear_triangular(nx, ny)

    def solve_linear_triangular(self, nx: int, ny: int) -> BivariateFunction:
        ...
        # Build global stiffness matrix K and load vector F
        ...
        # Collect boundary nodes and their prescribed values
        boundary = {}
        for i in range(nx + 1):
            for j in range(ny + 1):
        ...
        # Move boundary contributions to the right-hand side for interior nodes
        for k, gk in boundary.items():
            for m in range(n_total):
                if m not in boundary:
                    F[m] -= K[m][k] * gk

        # Enforce Dirichlet conditions: zero rows/columns, unit diagonal
        for k, gk in boundary.items():
            for m in range(n_total):
                K[k][m] = 0.0
                K[m][k] = 0.0
        ...
        b_vec = Vector(*F)
        sol = LinearSystem(A_mat, b_vec).solve(method='gauss_elimination')
        u = sol.components

        # Return a BivariateFunction using bilinear interpolation on the mesh
        xa_ = self.xa
        ya_ = self.ya
        ...
        def eval_u(x, y):
            ...
            return (1 - lx) * (1 - ly) * u00 + lx * (1 - ly) * u10 \
                 + (1 - lx) * ly        * u01 + lx * ly        * u11

        return BivariateFunction(eval_u)

    def _add_element(self, K, F, verts, idxs):
        """
        Add element stiffness and load contributions to the global arrays.

        verts: list of 3 (x, y) vertex coordinates
        idxs:  corresponding global node indices
        """
        x1, y1 = verts[0]
        x2, y2 = verts[1]
        x3, y3 = verts[2]
        ...
        # Shape-function gradient coefficients (constant per element)
        b = [y2 - y3, y3 - y1, y1 - y2]
        c = [x3 - x2, x1 - x3, x2 - x1]

        for i in range(3):
            # Element stiffness: K_e[i][j] = (b[i]*b[j] + c[i]*c[j]) / (4*area)
            for j in range(3):
                K[idxs[i]][idxs[j]] += (b[i] * b[j] + c[i] * c[j]) / (4 * area)
            # Element load (vertex quadrature): F_e[i] = (area/3) * f(x_i, y_i)
